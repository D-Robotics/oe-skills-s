...
class ReleaseContractTests(unittest.TestCase):
    ...
    def test_every_s_skill_has_v1_release_frontmatter(self):
        ...
        for path in skill_paths:
            ...
            self.assertRegex(header, r"(?m)^version:\s*1\.0\.2\s*$", path)
            self.assertRegex(header, r"(?m)^license:\s*Apache-2\.0\s*$", path)

    def test_source_version_is_v1_release(self):
        self.assertEqual((ROOT / "drobotics-s" / "VERSION").read_text(encoding="utf-8").strip(), "1.0.2")
    ...
    def test_skill_index_versions_match_indexed_skill_frontmatter(self):
        ...
        for name, entry in index["paths"].items():
            ...
            self.assertEqual(entry["version"], "1.0.2", name)
    ...
    def test_setup_records_the_requested_release_ref(self):
        with tempfile.TemporaryDirectory(dir=ROOT) as temporary_directory:
            ...
            result = subprocess.run(
                ["bash", "setup.sh", "--ref", "v1.0.2", shell_path(project)],
                cwd=ROOT,
                check=True,
                text=True,
                encoding="utf-8",
            ...
            self.assertEqual((project / ".drobotics-s" / "VERSION").read_text(encoding="utf-8").strip(), "1.0.2")
            self.assertEqual((project / ".drobotics-s" / "INSTALLED_REF").read_text(encoding="utf-8").strip(), "v1.0.2")
    ...
    def test_setup_update_treats_a_crlf_version_as_current(self):
        with tempfile.TemporaryDirectory(dir=ROOT) as temporary_directory:
            ...
            (destination / "VERSION").write_bytes(b"1.0.2\r\n")
            ...
            self.assertIn("Already up to date (1.0.2)", result.stdout)

...
| 链路 | 编译工具 | 说明 |
|------|---------|------|
| **标准 PTQ**（浮点 ONNX/Caffe → 量化 HBM） | `hmct-workflow` / `horizon-tc-ui` | `hb_config_generator` 生成官方 YAML，再运行 `hb_compile -c <config.yaml>` |
| **QAT**（`qat.bc` → HBM） | `s-hbdk-compile` | 仅适用于 QAT BC；走自定义工作流与用户确认门禁 |
| 仅检查模型（不做 PTQ） | `hmct-workflow` / `horizon-tc-ui` | `hb_compile -m <model> --march <march>` |
...
| 从已有 PTQ YAML 配置执行标准编译 | `horizon-tc-ui` | `hb_compile -c <config.yaml>` |
...
| 检查模型能否被工具链处理（不执行 PTQ） | `hmct-workflow` / `horizon-tc-ui` | `hb_compile -m <model> --march <march>` |
...
> - 涉及普通 PTQ/QAT 量化编译（.onnx/.bc → .hbm）→ 普通 PTQ 用 `hmct-workflow` / `horizon-tc-ui` 的官方 YAML + `hb_compile`；QAT `qat.bc` 用 `s-hbdk-compile`，走 OE 包检查

...
* Toolchain Routing and Orchestration: `drobotics-router` serves as the top-level entry point, dispatching requests to the appropriate sub-skill based on the `description` field of each skill in `skill-index.json`.
...
<td>s-hmct-cosine-similarity-tuning</td>
</tr>
<tr>
...
<td>s-plugin-precision-tuning</td>
</tr>
<tr>
...
<td>s-plugin-model-check-result</td>
</tr>
<tr>
...
<td>s-plugin-hbdk-generating</td>
</tr>
<tr>
...
| --------------- | ---------------- | ------------------------------------- |
| s-hbdk-compile | Advanced custom compilation (QAT `qat.bc` / explicitly requested workflows) | QAT BC compilation and specialized inputs/reports; ordinary PTQ uses official `hb_config_generator` + `hb_compile -c` |
| hbdk-manual     | HBDK4 compilation tool usage guide index | Querying usage of compilation tools by task scenario                      |

### D Robotics Plugin Module (PyTorch Quantization)
...
| s-plugin-graph-diff         | FX Graph computational graph diff comparison       | Locating structural and operator parameter differences between two graphs                          |
| s-plugin-consistency-debug  | Training‑deployment consistency issue diagnosis           | QAT training normal but export/convert/compile/HBM accuracy drops    |
| s-plugin-precision-tuning   | PyTorch‑side accuracy tuning          | Calibration accuracy below target, QAT loss not converging, mixed‑precision tuning     |
...
| Skill                          | Function                           | Trigger Scenarios                                             |
| ------------------------------ | ---------------------------- | ------------------------------------------------ |
| s-ucp-infer-generating        | UCP inference C++ code generation              | UCP/DNN inference API usage, parameter details, model loading, tensor memory, Cache synchronization    |
| s-ucp-hbm-infer               | hbm\_infer Python client code generation    | X86‑side Python HBM model inference via HbmRpcSession/HTensor   |
| s-ucp-model-perf-eval         | hrt\_model\_exec perf on‑board performance evaluation | Model performance testing, benchmarking, thread\_num/core\_id scanning, throughput/latency comparisons |
| s-ucp-perfetto-trace-catcher  | Perfetto trace capture on board          | Capturing UCP `.pftrace`, RDK S series dev board trace collection                 |
| s-ucp-perfetto-trace-analysis | Perfetto trace performance bottleneck analysis        | UCP inference trace diagnostics, latency/pipeline stall/BPU gap analysis            |
| s-board-monitor               | On‑board resource monitoring and collection                    | BPU utilization, DDR bandwidth, memory usage, resource monitoring during LLM inference                 |

### D Robotics TC UI Module (Visualization Analysis)
